  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
    ...
    try {
      ...
      if (action.equals("getVersionCode")) {
        PackageManager packageManager = this.cordova.getActivity().getPackageManager();
        int vs=packageManager.getPackageInfo(this.cordova.getActivity().getPackageName(), 0).versionCode;
        LOG.d(CordovaActivity.TAG, "CordovaActivity.onCreate()vvv11:"+vs);
        callbackContext.success(vs);
      return true;
      }
      return false;
